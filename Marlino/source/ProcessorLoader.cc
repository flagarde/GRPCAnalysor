...
ProcessorLoader::ProcessorLoader(lcio::StringVec::const_iterator first, lcio::StringVec::const_iterator last ) 
{
  ...
  while( current != last )
  {
    ...
    if( _checkDuplicateLibs.find( libBaseName ) == _checkDuplicateLibs.end() ) _checkDuplicateLibs.insert( libBaseName ) ;
    else
    {
      std::cout << std::endl <<red<< "<!-- ERROR loading shared library : " << libName << std::endl
      << "    ->    Trying to load DUPLICATE library -->" << normal<<std::endl << std::endl ;
      _loadError=true;
    }
    if( ! _loadError )
    {
      ...
      {
        std::cout << std::endl <<red<< "<!-- ERROR loading shared library : " << libName << std::endl
        << "    ->    "   << dlerror() << " -->" <<normal<< std::endl << std::endl ;
        _loadError=true;
      }
      else
      {
