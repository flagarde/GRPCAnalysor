...
namespace IMPL 
{
  ...
  LCEventImpl::LCEventImpl() :_runNumber(0),_eventNumber(0),_timeStamp(0),_detectorName("unknown") {}
  LCEventImpl::~LCEventImpl() 
  {
    ...
    for ( LCI i=_colMap.begin() ; i != _colMap.end() ; i++ )
    {
      ...
      if( _notOwned.find( i->second ) ==  _notOwned.end() )delete i->second ;
    }
  }
  ...
  LCCollection * LCEventImpl::getCollection(const std::string & name) const throw (DataNotAvailableException, std::exception) 
  {
  ...
  LCCollection * LCEventImpl::takeCollection(const std::string & name) const throw (DataNotAvailableException, std::exception) 
  {
  ...
  void  LCEventImpl::addCollection(LCCollection * col, const std::string & name) throw (EventException, std::exception)  
  {
    if( ! validateCollectionName(name.c_str()) )
    {
      throw EventException( std::string("LCEventImpl::addCollection() invalid name (has to be C/C++ name): "+name) ) ; 
    }  
    // check if name exists
    if( _colMap.find( name ) != _colMap.end() )throw EventException( std::string("LCEventImpl::addCollection() name already exists: "+name) ) ; 
    // check if col != 0
    if( col == 0  )
    throw EventException( std::string("LCEventImpl::addCollection()  cannot add NULL collection for : "+name) ) ; 
    _colMap[ name ]  = col ;
  }
  ...
  */
  bool LCEventImpl::validateCollectionName( const char* name )
  { 
    //copy of SIO_functions::validateName
