...
{       
	...
	{
        Addone=1;
        addnbr="Distribution Nbr Hits in Plane : "+std::to_string(NbrPlatee+1);
        addnbr2="Distribution timestamps in Plane : "+std::to_string(NbrPlatee+1);
        std::string a = "Distribution Nbr Hits in Plane : "+std::to_string(NbrPlatee+1);
        std::string a2 = "Distribution timestamps in Plane : "+std::to_string(NbrPlatee+1);
        ...
        for(int i=0;i<_Difs_Names.size();++i)
	{
		Calibration.insert(std::pair<int,std::array<std::array<double,64>, 48>>(_Difs_Names[i],std::array<std::array<double,64>, 48>{}));
                if(Distr==true)
		{
			addnbr="Distribution Nbr Hits in Dif : "+std::to_string(_Difs_Names[i])+" Plane : "+std::to_string(NbrPlatee+1);
                        addnbr2="Distribution timestamps in Dif : "+std::to_string(_Difs_Names[i])+" Plane : "+std::to_string(NbrPlatee+1);
			std::string a = "Distribution Nbr Hits in Dif : "+std::to_string(_Difs_Names[i]);
                        std::string a2 = "Distribution timestamp in Dif : "+std::to_string(_Difs_Names[i]);
			...
			for(int j=1;j<=48;++j)
			{
				std::vector<int>vec{_Difs_Names[i],j};
                        	addnbr="Distribution Nbr Hits in Asic : "+std::to_string(j)+"Dif : "+std::to_string(_Difs_Names[i])+" Plane : "+std::to_string(NbrPlatee+1);
                                addnbr2="Distribution timespamp in Asic : "+std::to_string(j)+"Dif : "+std::to_string(_Difs_Names[i])+" Plane : "+std::to_string(NbrPlatee+1);
				std::string b = "Distribution Nbr Hits in Asic : "+std::to_string(j);
                                std::string b2 = "Distribution timestamps in Asic : "+std::to_string(j);
				if (gROOT->FindObject(addnbr.c_str()) != NULL)continue;
				...
				for(int k=0;k<=63;++k)
		        	{
  					std::vector<int>vec{_Difs_Names[i],j,k};
                        		addnbr="Distribution Nbr Hits in Pad : "+std::to_string(k)+"Asic : "+std::to_string(j)+"Dif : "+std::to_string(_Difs_Names[i])+" Plane : "+std::to_string(NbrPlatee+1);
					addnbr2="Distribution timestamp Hits in Pad : "+std::to_string(k)+"Asic : "+std::to_string(j)+"Dif : "+std::to_string(_Difs_Names[i])+" Plane : "+std::to_string(NbrPlatee+1);
					std::string c = "Distribution Nbr Hits in Pad : "+std::to_string(k);
					std::string c2 = "Distribution timestamp Hits in Pad : "+std::to_string(k);
					if (gROOT->FindObject(addnbr.c_str()) != NULL)continue;
	...
	{
                addnbr=(*it)+std::to_string(NbrPlate);
                if (gROOT->FindObject(addnbr.c_str()) != NULL) continue;
        ...
        for(std::vector< std::basic_string<char>  >::iterator it=vec_name_th2_Asic.begin();it!=vec_name_th2_Asic.end();++it)
	{
       ...
       // if (TH1Fs.size()!=0&&TH2Fs.size()!=0) std::cout<<red<<"Creating "<<TH1Fs.size()+Addone+Difs_Distr.size()+Asics_Distr.size()+Pads_Distr.size()<<" TH1 and "<<TH2Fs.size()<<" TH2F for plate "<<std::to_string(NbrPlate+1)<<normal<<std::endl;
}
...
void HistoPlane::WriteAll(int NbrPlatee,std::string namepdf)
{
  ...
  std::string name="Title:Plate "+ std::to_string(NbrPlatee+1);
  for(std::map<std::string,TH1D*>::iterator it=TH1Fs.begin();it!=TH1Fs.end();++it)
  {
...
void HistoPlane::Save(TFile* file,std::string namepdf)
{
    ...
    ugly->Write();
    delete ugly;
    std::string plate="Plate "+ std::to_string(NbrPlatee+1);
