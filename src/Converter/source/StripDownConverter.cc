...
{
  return 2*(64-Channel)-1+geom->GetDifPositionX(Dif_Id);
}
...
std::vector<float> StripDownConverter::IJKToXYZ(int I,int J,int K)
{
...
std::vector<int> StripDownConverter::RawToIJK(int Dif_Id,int Asic_Id,int Channel)
{
  return {RawToI(Dif_Id,Asic_Id,Channel),RawToJ(Dif_Id,Asic_Id,Channel),RawToK(Dif_Id,Asic_Id,Channel)};
}

std::vector<int> StripDownConverter::IJKToRaw(int I,int J,int K)
{
  return {0,0,0};
}
