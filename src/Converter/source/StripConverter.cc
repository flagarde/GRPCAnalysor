...
int StripConverter::RawToI(int Dif_Id,int Asic_Id,int Channel)
{
  if(geom->GetDifUpDown(Dif_Id)==1) return (2*Channel)+geom->GetDifPositionX(Dif_Id);
  else return 2*(64-Channel)-1+geom->GetDifPositionX(Dif_Id);
}

int StripConverter::RawToJ(int Dif_Id,int Asic_Id,int Channel)
{
  return Asic_Id;
}

int StripConverter::RawToK(int Dif_Id,int Asic_Id,int Channel)
{
  return geom->GetDifNbrPlate(Dif_Id);
}

std::vector<double> StripConverter::IJKToXYZ(int I,int J,int K)
{
...
std::vector<int> StripConverter::RawToIJK(int Dif_Id,int Asic_Id,int Channel)
{
  return {RawToI(Dif_Id,Asic_Id,Channel),RawToJ(Dif_Id,Asic_Id,Channel),RawToK(Dif_Id,Asic_Id,Channel)};
}

std::vector<int> StripConverter::IJKToRaw(int I,int J,int K)
{
  return {0,0,0};
}
